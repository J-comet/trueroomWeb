<?php
  ...
  $page = $_GET['page'];
  $category = $_GET['category'];  //검색 카테고리
  $search_value = $_GET['search'];  // 검색 단어

  $email = $_SESSION['useremail'];

  $connect = mysqli_connect("localhost","root","gptjd7848","trueroom") or die("fail");

  $sql = "SELECT * FROM member WHERE email ='{$email}'"; // email 로 해당 email 찾음
...
if ($category == "name") {
  $category_title = "닉네임";
} elseif ($category == "company_name") {
  $category_title = "기업명";
} elseif ($category == "what_job") {
  $category_title = "직종";
} elseif ($category == "where_work") {
  $category_title = "근무지역";
}

 ?>
<div style="width:700px; margin: 0 auto;">
  <h2 class="text-center">[<?php echo $category_title; ?>] &nbsp&nbsp"<?php echo $search_value; ?>"&nbsp&nbsp (으)로 검색결과</h2><br><br>
 ...
 <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="get">
   ...
   <div style="width:80%; height:40px; border:0px solid skyblue; float:left; margin-right:2px">
     ...
     <select name="category" style="float:left;">
       <option value="company_name" <?php if($category=="company_name"):?> selected <?php endif ?>>기업명</option>
       <option value="name" <?php if($category=="name"):?> selected <?php endif ?>>닉네임</option>
       <option value="what_job" <?php if($category=="what_job"):?> selected <?php endif ?>>직종</option>
       <option value="where_work" <?php if($category=="where_work"):?> selected <?php endif ?>>근무지역</option>
     </select>


     <div style="margin-left: 5px;width: 80%; height: auto;float:left; border:1px solid skyblue;">
     <input type="text" name="search" value="<?php echo $search_value; ?>" style="font-size:16px; width:355px; height: 35px;padding:8px 0 4px 10px; border:0px; outline:none; float:left" required>
...
if (isset($search_value)) {   // 검색 단어가 입력되었다면
    $query = "SELECT * FROM reviewinfo WHERE $category like '%$search_value%'";    // reviewinfo 의 카테고리에 맞는 검색단어를 찾음
} else {
...
// 실제 주소데이터 가져오기
$query1 ="SELECT * FROM reviewinfo WHERE $category like '%$search_value%' order by idx desc limit $start,$view_article"; // idx 를 기준으로 내림차순정렬
mysqli_query($connect,"set names utf8");   // set names utf8 을 붙히지 않으면 한글이 깨져서 저장됨
$result = mysqli_query($connect,$query1);
//$total = mysqli_num_rows($result); //  검색 게시글 총개수
...
<div class="postleft">
<p style="font-size:21px; color:black; height:33px; overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"><?php echo $company_name; ?></p>
...
if ($total_article == 0) {
  ?>
  <center><h3 style="margin-bottom:300px;">검색결과가 없습니다</h3></center>
...
// 처음페이지로 이동 //
if ($page != 1) {
echo "<li class='page-item'><a class='page-link' href=$PHP_SELF?page=1&category=$category&search=$search_value><<</a></li>";
} else {
...
// 1. 이전 페이지 그룹이동버튼 //
if ($page != 1) {
  echo "<li class='page-item'><a class='page-link' href = $PHP_SELF?page=$prev_group&category=$category&search=$search_value>이전</a></li>";
} /* else {
...
for($i=$start_page; $i < $end_page; $i++){
  ...
  if($i==$page){  //현재 출력된 게시글과 페이지 개수가 같다면
    echo "<li class='page-item active'><a class='page-link'>$i</a></li>";
       ...
       echo "<li class='page-item'><a class='page-link' href=$PHP_SELF?page=$i&category=$category&search=$search_value>$i</a></li>";
     }
}
...
if ($page <=($total_page + $page - $end_page)) {
  echo "<li class='page-item'><a class='page-link' href=$PHP_SELF?page=$next_group&category=$category&search=$search_value>다음</a></li>";
}


// 마지막페이지로 이동 //
if ($page < $total_page) {
  echo "<li class='page-item'><a class='page-link' href=$PHP_SELF?page=$total_page&category=$category&search=$search_value>>></a></li>";
} else {
  echo "<li class='page-item disabled'><a class='page-link'>>></a></li>";
}
...
<?php } ?>
    ...
    <br><br>
      ...
      <footer class="py-5 bg-primary">
        <div class="container">
          <p class="m-0 text-center text-white">Copyright &copy; (주)진실의방 2019</p>
        </div>
      </footer>
