...
if (preg_match('/^[가-힣]+$/',$name) == false) {
    echo "<script>alert('한글만가능'); history.back();</script>";
}
...
elseif ($rowname != $name) {   // 바꾸려는 닉네임이 현재 내 닉네임이 아니라면 중복검사를 실시
  if($name_check >= 1){
    ...
    $sql = "update member set name='$name',gender='$gender', company_name='$company_name', what_service='$what_service' where email='$email'";

     $result = $connect->query($sql);  // $result 는 member 테이블의 업데이트 성공했는지 확인하는 결과값

     mysqli_query($connect,"set names utf8");
     $sql2 = "update reviewinfo set name='$name',gender='$gender', company_name='$company_name' where member_idx='$idx'";
