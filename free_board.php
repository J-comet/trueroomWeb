      ...
      -->
         ...
         <?php } else {
           ...
           <ul class="nav navbar-nav navbar-right">
             <li class="nav-item active" >
                 <strong>
                   <a id="gnb" class="nav-link"><img class="rounded" src="image/user.png" style="width:30px;height:30px;margin-right:15px;"><?php echo "$name 님"; ?>
...
<span>
  <!--
<button type="button" name="button" class="btn btn-success" disabled>번호순</button>
<button type="button" name="button" class="btn btn-outline-success">조회순</button>
  <button type="button" name="button" class="btn btn-outline-success">추천순</button>
-->
...
$query = "select * from free_board";    // free_board 의 모든 데이터를 조회
$result = mysqli_query($connect,$query);
$total_article = mysqli_num_rows($result);  // 총 게시물 개수

$view_article = 10;  // 한페이지에 몇개의 게시글 보이게 할지 결정
if (!$page)$page=1;  // 페이지번호가 없다면 페이지 번호 1을 만듬
$start = ($page-1) * $view_article; // 10개씩 보여줄 시작점

$cnt=1; // 글 번호를 생성
$cnt=$total_article - ($view_article * ($page - 1));

// 실제 주소데이터 가져오기
$query1 ="SELECT * FROM free_board order by idx desc limit $start,$view_article"; // idx 를 기준으로 내림차순정렬
mysqli_query($connect,"set names utf8");   // set names utf8 을 붙히지 않으면 한글이 깨져서 저장됨
...
if ($total_article == 0) {
   ...
   <!-- <img src="image/user.png" class="card-img-top img-fluid" style="width:25px; height:25px;"> -->
     <div class="card">
           ...
           <div class="card-body" style="height:250px;">
             <p style="padding:80px 0 0 400px; font-weight:bold; font-size:30px;">작성된 게시글이 없습니다</p>
           </div>
  ...
  <table class="table table-bordered" style="text-align:center">
    ...
    <tbody>
     ...
     <a href="free_board_view.php?idx=<?php echo $boadr_idx; ?>&title=<?php echo $title; ?>">
       <?php

       // 게시물에 댓글이 있다면 댓글수를 출력
        $reply_SQL = "SELECT * FROM reply WHERE board_idx='".$boadr_idx."'";
        mysqli_query($connect,"set names utf8");   // set names utf8 을 붙히지 않으면 한글이 깨져서 저장됨
  ...
  <div style="width:100%">
    ...
    if (isset($email)) {
       ?>
      <a href="free_board_write.php" class="btn btn-primary" style="width: 80px; height: 50px; padding-top: 12px;float:right; margin-bottom:30px;" ><strong>글작성</strong></a>
    <?php } else {
      ?>
      <a href="check_login.php" class="btn btn-primary" style="width: 80px; height: 50px; padding-top: 12px;float:right; margin-bottom:30px;" ><strong>글작성</strong></a>

    <?php   } ?>
  </div>

  <ul class="pagination pagination-lg justify-content-center" style="margin-top:100px;">
...
<?php }  // 작성된 게시글이 있을때 작동하는 else 문 끝나는 지점  ?>
    ...
    <br><br>
      <!-- Footer -->
      <footer class="py-5 bg-primary" style="margin-top:50px;">
        <div class="container">
