<?php
  ...
  $query = "SELECT * FROM member WHERE email ='{$email}'"; // email 로 로그인한 유저의정보 찾음
...
<center>
  ...
  <ul class="nav nav-tabs nav-justified" style="width:80%">
    <li class="nav-item">
      <a class="nav-link" href="freeboard_post.php">소통공간 게시글</a>
    </li>
    <li class="nav-item">
      <a class="nav-link active" href="my_review_write_post.php">작성한 댓글</a>
    </li>
    <!--
    <li class="nav-item">
...
<div style="width:80%; margin:0 auto">
  ...
  $total_query = "SELECT * FROM reply WHERE user_idx='".$user_idx."'";
  mysqli_query($connect,"set names utf8");   // set names utf8 을 붙히지 않으면 한글이 깨져서 저장됨
  ...
  //$freeboard_query = "SELECT * FROM free_board WHERE name = '{$name}' order by idx desc limit 0,5";
  $reply_query = "SELECT * FROM reply WHERE user_idx = '{$user_idx}' order by idx desc limit $start,$view_article";
  ...
  $reply_result = mysqli_query($connect,$reply_query);
    ...
    if ($total_free_article == 0) {   // 작성된 최신 댓글이 없을때 코드 실행
 ?>
 <div class="card">
   <div class="card-body" style="height:200px;">
     <p style="padding:60px 50px 50px 450px; font-weight:bold; font-size:30px;">작성한 댓글이 없습니다</p>
   </div>
 ...
 while ($rows = mysqli_fetch_assoc($reply_result)) {  //DB 에 저장된 데이터 수 (열 기준)

   $replyIDX = $rows['idx'];
   $freeboard_idx = $rows['board_idx'];
   //$name = $rows['name'];
   //$title = $rows['title'];
   ...
   // 게시글의 제목을 찾는 작업
   $board_SQL = "SELECT * FROM free_board WHERE idx='".$freeboard_idx."'";
   $board_result = mysqli_query($connect,$board_SQL);
   mysqli_query($connect,"set names utf8");   // set names utf8 을 붙히지 않으면 한글이 깨져서 저장됨
   $board_result = $connect->query($board_SQL);

   if($board_result->num_rows >= 1) {   // 쿼리문 실행결과가 있을 때 실행

     $row = mysqli_fetch_array($board_result);
     $board_title = $row['title']; // 게시글의 고유 idx 로 찾은 게시글 제목
...
<form action="reply_delete.php" method="post">
  ...
  <div class="custom-control custom-checkbox">
  <input type="checkbox" name="chk_delete[]" class="check custom-control-input chk_select" id="<?php echo "customCheck".$plus; ?>" value="<?php echo $replyIDX; ?>">
  <label class="custom-control-label" for="<?php echo "customCheck".$plus; ?>"></label>
</div>
</td>

 <td width='250' height='15'>
<a href="free_board_view.php?idx=<?php echo $freeboard_idx; ?>&title=<?php echo $board_title; ?>">
  <?php
    echo $content;
   ?>
</a>
 ...
 ?>
  ...
  <td width='80'>
    <a class="btn btn btn-info btn-sm" href="free_board_view.php?idx=<?php echo $freeboard_idx; ?>&title=<?php echo $board_title; ?>">이동</a>
  </td>
</tbody>
<?php
   }
